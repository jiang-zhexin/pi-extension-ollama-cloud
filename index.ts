...
export default function (pi: ExtensionAPI) {
      ...
      const models = await Promise.all(
        list.models.map(async (m): Promise<ProviderModelConfig> => {
          ...
          return {
            ...
            maxTokens: 32768,
            thinkingLevelMap: toThinkingMap(show.thinking),
          };
        }),
      );
  ...
  pi.on("resources_discover", async (_event, ctx) => {
    const ollamaApiKey = await ctx.modelRegistry.getApiKeyForProvider(
      "ollama-cloud",
    );
...
function toThinkingMap(
  ...
  const thinkingLevels = thinking?.values;
  if (!thinkingLevels) return;

  const thinkingLevelSet = new Set(thinkingLevels);

  if (thinkingLevelSet.has(true)) thinkingLevelMap.minimal = "minimal";
  if (thinkingLevelSet.has("low")) thinkingLevelMap.low = "low";
  if (thinkingLevelSet.has("medium")) thinkingLevelMap.medium = "medium";
  if (thinkingLevelSet.has("high")) thinkingLevelMap.high = "high";
  if (thinkingLevelSet.has("max")) thinkingLevelMap.max = "max";
